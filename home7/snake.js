...
var fieldSizeX  = 20;
var fieldSizeY = 20;
var snakeSpeed = 100;      // Интервал между перемещениями змейки
var foodSpeed = 5000;      // Интервад между созданием еды
var problemSpeed = 5000;   // интервал межуд созданием препятствий
...
function init() {
    ...
    if (16 * (fieldSizeX + 1) < 380) {
        conteiner.style.width = '380px';
    }                                                                   
    else {
        conteiner.style.width = (16 * (fieldSizeX + 1)).toString() + 'px';
    }
...
function startGame() {
    if (!gameIsRunning) {
        ...
        food_timer = setInterval(createFood, foodSpeed);
        problem_timer = setInterval(createProblem, problemSpeed);
    }
}
...
function respawn() {
    ...
    var start_coord_x = Math.floor(fieldSizeX / 2);
    var start_coord_y = Math.floor(fieldSizeY / 2);
...
function headTeleport(coord_y, coord_x) {
    var unit;
    if (direction == 'x-') {
        unit = document.getElementsByClassName('cell-' + (coord_y) + '-' + (fieldSizeX-1))[0];
    }
    else if (direction == 'x+') {
        unit = document.getElementsByClassName('cell-' + (coord_y) + '-' + (0))[0];
    }
    else if (direction == 'y+') {
        unit = document.getElementsByClassName('cell-' + (fieldSizeY-1) + '-' + (coord_x))[0];
    }
    else if (direction == 'y-') {
...
function createFood() {
    ...
    while (!foodCreated) {
        // рандом
        var food_x = Math.floor(Math.random() * fieldSizeX);
        var food_y = Math.floor(Math.random() * fieldSizeY);
...
function createProblem() {
    ...
    while (!problemCreated) {
        // рандом
        var problem_x = Math.floor(Math.random() * fieldSizeX);
        var problem_y = Math.floor(Math.random() * fieldSizeY);
